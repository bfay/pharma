...
*** Modified to work with WordPress Multisite by LukeAmerica (wpCodeSnippets.info & FiveRingsMedia.com)
 */
...
$this->error("Cache File: {$this->cachefile}");
$this->error("docRoot: {$this->docRoot}");
				$this->set404();
				return false;
			}
	...
	protected function calcDocRoot(){
		...
		if (is_multisite()){
			...
			if (isset($blog_id) && $blog_id > 0){
				...
				$pos = strpos($_GET['src'], '/wp-content/themes/');
				if ($pos === false){
